...
class PlaneNormalVisualizer {
 public:
  ...
  void UpdateCloud(std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> v_cloud, 
                    std::vector<std::string> v_cloud_name,
                    std::vector<std::vector<double>> v_cloud_color) {
    for(int i=0; i < v_cloud.size(); i++) {
      UpdateCloud(v_cloud[i],
                  v_cloud_name[i],
                  v_cloud_color[i]);
    }
  ...
  void UpdateCloudAndNormals(std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> v_cloud,
                             std::vector<pcl::PointCloud<pcl::Normal>::Ptr> v_normals,
                             int ds_ratio ,
                             std::vector<std::string> v_cloud_name ,
                             std::vector<std::string> v_normals_name ,
                             std::vector<std::vector<double>> v_cloud_color,
                             std::vector<std::vector<double>> v_normals_color) {
    for(int i=0; i < v_cloud.size(); i++) {
      UpdateCloudAndNormals(v_cloud[i], 
                            v_normals[i], 
                            ds_ratio, 
                            v_cloud_name[i],
                            v_normals_name[i],
  ...
  void UpdateLines(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud1,
                   ...
                   bool clean_screen_flag = true);

  void UpdateLines(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud1,
                   pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud2,
                   std::vector<std::vector<double>> v_line_color);

  void UpdateLines(std::vector<std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr>> v_line_cloud,
                  std::vector<std::vector<double>> v_line_color) 
  {
    pcl::PointCloud<pcl::PointXYZ>::Ptr all_start_pts(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr all_stop_pts(new pcl::PointCloud<pcl::PointXYZ>);
    std::vector<std::vector<double>> all_line_colors;
    for(int i=0; i < v_line_cloud.size(); i ++) {
      *all_start_pts += *v_line_cloud[i].first;
      *all_stop_pts += *v_line_cloud[i].second;
      for(int j = 0; j < v_line_cloud[i].first->size(); j++) {
        all_line_colors.push_back(v_line_color[i]);
      }
    }
    UpdateLines(all_start_pts,
                all_stop_pts,
                all_line_colors);     
  }
